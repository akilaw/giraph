 ...
 */
public class HiveOutputIntNullEdge extends SimpleEdgeToHive<IntWritable,
  ...
  @Override public void fillRecord(IntWritable sourceId,
                                   IntWritable sourceValue,
                                   Edge<IntWritable, NullWritable> edge,
                                   HiveWritableRecord record) {
    record.set(0, (long) sourceId.get());
    record.set(1, (long) edge.getTargetVertexId().get());
    System.out.println("HiveOutputIntNullEdge:" + sourceId.get() + "," +
