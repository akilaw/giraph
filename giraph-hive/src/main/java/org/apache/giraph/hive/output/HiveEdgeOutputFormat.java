...
import static org.apache.giraph.hive.common.HiveUtils.newEdgeToHive;
 ...
 * @param <E> Edge Value
 */
public class HiveEdgeOutputFormat<I extends WritableComparable,
   ...
   */
  public HiveEdgeOutputFormat() {
    hiveOutputFormat =
        new HiveApiOutputFormat();
  }
  ...
  private HiveOutputDescription makeOutputDesc() {
    ...
    tableDesc.setDatabaseName(HIVE_EDGE_OUTPUT_DATABASE.get(getConf()));
    tableDesc.setTableName(HIVE_EDGE_OUTPUT_TABLE.get(getConf()));
    outputDesc.setPartitionValues(
        parsePartitionValues(HIVE_EDGE_OUTPUT_PARTITION.get(getConf())));
    return outputDesc;
  }
  ...
  public void setConf(
    ...
    hiveOutputFormat.initialize(makeOutputDesc(),
        HIVE_EDGE_OUTPUT_PROFILE_ID.get(conf), conf);
  }

  @Override
  public EdgeWriter<I, V, E> createEdgeWriter(TaskAttemptContext context)
    throws IOException, InterruptedException {
    RecordWriter<WritableComparable, HiveWritableRecord> baseWriter =
        hiveOutputFormat.getRecordWriter(context);
    HiveEdgeWriter<I, V, E> writer = new HiveEdgeWriter<I, V, E>();
  ...
  public void checkOutputSpecs(JobContext context)
    ...
    EdgeToHive<I, V, E> edgeToHive = newEdgeToHive(getConf(), schema);
    edgeToHive.checkOutput(makeOutputDesc(), schema,
